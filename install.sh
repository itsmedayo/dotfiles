...
if [[ "${user}" == "itsme" && "${hostname}" == "home-"* ]]; then
    ...
    rename -- 'itsme.' "${user}-local." "/mnt/var/cache/pacman/${user}-local"/*
else
...
if [ "${user}" = "itsme" ]; then
    echo -e "\n### Cloning dotfiles"
    arch-chroot /mnt sudo -u $user bash -c 'git clone --recursive https://github.com/itsme/dotfiles.git ~/.dotfiles'
